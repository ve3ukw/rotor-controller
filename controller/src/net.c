...
static void tcp_receive(const sm_ctx_t *sm)
{
    ...
    if (!skip_normal_read) {
        ...
        /* Read byte-by-byte with individual CS cycles.
           This chip variant produces a 1-bit right-shift when the RX buffer
           is read as a burst (BSB=3, WIZCHIP_READ_BUF); individual WIZCHIP_READ
           calls (each with its own CS assert/deassert) give correct data. */
        /* BSB=3 reads are 1-bit right-shifted on this chip variant.
           Use 9-bit SSI mode to capture the extra dummy bit correctly. */
    ...
    while ((nl = (char *)memchr(line, '\n', (size_t)(g_rx_buf + g_rx_idx - line)))) {
        ...
        *nl = '\0';
        ...
        /* Bypass ioLibrary send() and getSn_TX_FSR() — both contain
           do-while stability loops that spin forever on this chip variant
           because its internal TX tracking causes register oscillation.
           Write directly to TX buffer and issue SEND with a bounded wait. */
        {
            uint16_t ack_len = (uint16_t)strlen(ack);
            wiz_send_data(SOCKNUM_TCP, (uint8_t *)ack, ack_len);
...
void net_tick(const sm_ctx_t *sm, float az_raw, float el_raw, uint32_t ts_ms)
{
    ...
    case SOCK_CLOSED:
        ...
        break;
    case SOCK_INIT: {
        /* Reset TX WR = RD (empty buffer) then issue a 0-byte SEND.
           This chip variant retains internal TX state across socket open/close
           that doesn't match the SPI-accessible registers; the 0-byte SEND
           forces the internal pointers to synchronise before we listen. */
        uint16_t txrd = getSn_TX_RD(SOCKNUM_TCP);
        setSn_TX_WR(SOCKNUM_TCP, txrd);
        setSn_CR(SOCKNUM_TCP, Sn_CR_SEND);
        while (getSn_CR(SOCKNUM_TCP)) {}   /* wait — completes immediately (0 bytes) */
        listen(SOCKNUM_TCP);
        break;
    }
    case SOCK_LISTEN:
    ...
    case SOCK_ESTABLISHED:
        if (!g_brain_connected) {
            ...
            /* Discard any stale TX buffer data from previous sessions:
               set TX write pointer = TX read pointer so the W5500 sees
               an empty TX buffer and won't flush old data on this connection. */
            setSn_TX_WR(SOCKNUM_TCP, getSn_TX_RD(SOCKNUM_TCP));
            debug_log("brain %u.%u.%u.%u\r\n",
    ...
    if (getSn_SR(SOCKNUM_UDP) == SOCK_CLOSED) {
        socket(SOCKNUM_UDP, Sn_MR_UDP, NET_UDP_PORT, 0);
    }

    /* Telemetry blast at 20 Hz */
    if (++g_telem_tick >= TELEM_INTERVAL_TICKS) {
        g_telem_tick = 0;
        if (g_brain_connected) {
            const char *frame = protocol_encode_telemetry(
                sm, az_raw, el_raw, ts_ms, g_telem_seq++);
            sendto(SOCKNUM_UDP, (uint8_t *)frame, (uint16_t)strlen(frame),
                   g_brain_ip, NET_UDP_PORT);
        }
    }
}
